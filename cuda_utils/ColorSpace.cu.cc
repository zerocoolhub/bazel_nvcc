...
__global__ static void OverlayImageKernel(BGRA32 *d_base_img_rgba, int d_base_img_width, int d_base_img_height, uchar4 *d_overlay_img_rgba, int d_overlay_width, int d_overlay_height)
{
    ...
    if (d_current_row < d_overlay_height && d_current_column < d_overlay_width) {
        int d_overlay_i = d_current_row * d_overlay_width + d_current_column;
        int d_base_i = (d_current_row * d_base_img_width) + d_current_column;

        uchar4 input_byte = d_overlay_img_rgba[d_overlay_i];
//        BGRA32 output_byte = (BGRA32)d_base_img_rgba[d_base_i];

//        output_byte.c.r = (unsigned char) input_byte.x;
//        output_byte.c.g = (unsigned char) input_byte.y;
//        output_byte.c.b = (unsigned char) input_byte.z;
//        output_byte.c.a = (unsigned char) input_byte.w;

        BGRA32 rgb{};
        rgb.c.r = input_byte.x;
        rgb.c.g = input_byte.y;
        rgb.c.b = input_byte.z;
        rgb.c.a = input_byte.w;

//        output_byte.c = rgb;
        d_base_img_rgba[d_base_i] = rgb;
//        *(BGRA32 *)output_byte = (uint) { rgb };
    }
}
...
{
    OverlayImageKernel
        <<<dim3(d_overlay_width / 32 + 1, d_overlay_height / 32 + 1), dim3(32, 32)>>>
        ((BGRA32 *)d_base_img_rgba, d_base_img_width, d_base_img_height, d_overlay_img_rgba, d_overlay_width, d_overlay_height);
}
...
void Bgra32ToNv12(uint8_t *dpBgra, int nBgraPitch, uint8_t *dpNv12, int nNv12Pitch, int nWidth, int nHeight, int iMatrix) {
    SetMatRgb2Yuv(iMatrix);
    RgbToYuvKernel<uchar2, BGRA32, uint2>
        <<<dim3((nWidth + 63) / 32 / 2, (nHeight + 3) / 2 / 2), dim3(32, 2)>>>
